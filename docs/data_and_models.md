...
`surrogate/best.pt` must describe `DeepwavePhysicsSurrogate`. The prepared bundle embeds its protocol hash; use the matching original snapshot for paper replay. `configs/paper/test_set.json` identifies the final evaluation models. The data directory layout and filenames must match the prepared bundle.
