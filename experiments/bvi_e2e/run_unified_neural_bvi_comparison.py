"""Six-method Neural-BVI evaluation with a shared scalar-wave forward operator."""
...
from runtime_integrity import input_hashes, fingerprint


HERE = Path(__file__).resolve().parent
DEFAULT_ROOT = HERE.parents[1] / "artifacts" / "paper"
DEFAULT_OUT = HERE.parents[1] / "outputs" / "paper"
...
def main() -> None:
    ...
    if not args.resume and out_dir.exists() and any(out_dir.iterdir()):
        raise ValueError("Output directory must be empty unless --resume is specified")
    ...
    artifact_hashes = input_hashes(root, [int(args.seed), *protocol["ensemble_seeds"]])
    code_hashes = {path.name: file_sha256(path) for path in sorted(HERE.glob("*.py"))}
    run_spec = {
        "input_sha256": artifact_hashes,
    ...
    basis_payload = build_error_pca_basis(
        ...
        device=device,
        input_fingerprint=fingerprint({"inputs": artifact_hashes, "code": code_hashes,
                                       "center": run_spec["ensemble_center_json_sha256"]}),
    )
