...
class PublicationProtocolTests(unittest.TestCase):
    ...
    def test_smoke_provenance_and_field_boundary(self) -> None:
        smoke_root = ROOT / "experiments" / "bvi_e2e" / "publication_la010010" / "smoke"
        manifest_path = smoke_root / "prepared" / "manifest.json"
        if not manifest_path.exists():
            self.skipTest("smoke pipeline has not been run")
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        if manifest["protocol_hash"] != self.protocol["protocol_hash"]:
            self.skipTest("smoke pipeline artifact is stale for the current protocol")
        self.assertTrue(manifest["leakage_guard"])
        self.assertEqual(manifest["protocol_hash"], self.protocol["protocol_hash"])
        field = json.loads((smoke_root / "field" / "summary.json").read_text(encoding="utf-8"))
        self.assertFalse(field["artificial_noise_added"])
        self.assertEqual(field["crop"]["mode"], "fixed_native_no_resampling")
