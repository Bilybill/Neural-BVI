...
def verify(directory):
    ...
    holdout = json.loads((ROOT / "configs/paper/test_set.json").read_text(encoding="utf-8"))
    identities = dict(zip(map(str, holdout["selected_global_indices"]),
                          holdout["selected_model_names"]))
    if models != set(identities) or any(
        row["model_name"] != identities.get(row["global_index"]) for row in rows
    ):
        raise ValueError("Records do not match the released test-model identities")
